...
def correlation_gpu(kernel, image):
    ...
    zero_padded_image = np.zeros((rows + (k_rows - 1), columns + (k_columns - 1)), dtype=image.dtype)
    ...
    dev_C = cuda.device_array(np.shape(image)) # create directly on gpu
...
def correlation_numba(kernel, image):
    ...
    k_rows, k_cols = kernel.shape
    rows, columns = image.shape
    zero_padded_image = np.zeros((rows + (k_rows - 1), columns + (k_cols- 1)), dtype=image.dtype)
